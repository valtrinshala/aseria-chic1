...
class ZReportInvoice
{
    ...
    public function getZReportDataForPrinter()
    {
        ...
        $shortVal = $this->zReport->closing_amount - (($this->zReport->orders()->where(['is_cancelled' => false])->sum('paid_cash') - $this->zReport->orders()->where(['is_cancelled' => false])->sum('payment_return')) + $this->zReport->saldo);
        ...
        return [
            ...
            'totals' => [
                'row_data' => [
                    ...
                    "1" => "space",
                    __('Closed') => is_string($this->zReport->end_z_report) ? (new DateTime($this->zReport->end_z_report))->format("d/m/Y - H:i A") : null,
                    "2" => "space",
                    "3" => "space",

                    __('Opening Amount') => number_format(floatval($this->zReport->saldo), $number_of_decimals, ".", ""),
                    __('Closing Amount') => number_format(floatval($this->zReport->closing_amount), $number_of_decimals, ".", ""),
                    $shortVal >= 0 ? __('Over') : __('Short') => number_format(floatval($shortVal), $number_of_decimals, ".", ""),
                    "4" => "space",
                    "5" => "space",

                    __('Cash Sales') => number_format(floatval($this->zReport->orders()->where(['is_cancelled' => false])->sum('paid_cash') - $this->zReport->orders()->where(['is_cancelled' => false])->sum('payment_return')), $number_of_decimals, ".", ""),
                    ...
                    __('Total Sales') => number_format(floatval($this->zReport->orders()->where('is_cancelled', false)->sum('payable_after_all')), $number_of_decimals, ".", ""),
                    __('Total Discounts') => '-'.number_format(floatval($this->zReport->orders()->where('is_cancelled', false)->sum('discount_amount')), $number_of_decimals, ".", ""),
                    __('Total Returns') => '-'.number_format(floatval($this->zReport->orders()->where('is_cancelled', true)->sum('payable_after_all')), $number_of_decimals, ".", ""),
                ],
            ],
            'sales_by_service' => [
                'row_data' => [
                    ...
                    'row_data_take_away' => [
                        __('Take away'),
                        count($this->zReport->orders()->where('is_cancelled', false)->where('order_type', 'take_away')->get()),
                        number_format(floatval($this->zReport->orders()->where('is_cancelled', false)->where('order_type', 'take_away')->sum('payable_after_all')), $number_of_decimals, ".", "")
                    ],
                    'row_data_dine_in' => [
                        __('Dine-in'),
                        count($this->zReport->orders()->where('is_cancelled', false)->where('order_type', 'dine_in')->get()),
                        number_format(floatval($this->zReport->orders()->where('is_cancelled', false)->where('order_type', 'dine_in')->sum('payable_after_all')), $number_of_decimals, ".", "")
                    ],
                    "1" => "space",
                    'row_data_total' => [
                        __('Total'),
...
//                    $this->tax::where('type', "dine_in")->first()?->name => number_format(floatval($this->zReport->orders()->where(['order_type' => 'dine_in', 'is_cancelled' => false])->sum('tax_amount')), $number_of_decimals, ".", ""),
                    ...
                    $paymentMethodCard->name => number_format(floatval($this->zReport->orders()->where('is_cancelled', false)->sum('paid_bank')), $number_of_decimals, ".", ""),
//                    "7" => "space",
//                    "8" => "space",
