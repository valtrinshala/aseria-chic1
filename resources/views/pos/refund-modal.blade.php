...
<div id="refund-confirmation" order="none" class="modal fade" data-backdrop="true">
    <div class="payment-loader hidden"></div>
    <div class="modal-dialog mh-100 mh-padd-3 my-0">
        <div class="modal-content p-10">
            <div>
                <h2 class="text-center">{{ __('Refund order') }}</h2>
            </div>
            <div>
                <p class="text-center">{{ __("This action cannot be able to undone, What is the reason behind refunding the order?") }}</p>
            </div>
            ...
            <div class="d-flex justify-content-between buttons-discount">
                <div class="">
                    <a href="#" class="btn font-span confirm-refund-order fw-bold py-3 tran-danger-bg border-danger text-danger">{{ __('Confirm refund') }}</a>
                </div>
                <div class="">
                    <a href="#" class="btn font-span discard-refund-order nfw-bold add-disc btn-violet py-3 gray-border text-white">{{ __('Discard this action') }}</a>
                </div>
            </div>
...
<script>
    ...
    function payment_complete(data) {
        ...
        paymentLoader.close();
        // console.log(data);
        // Honestly... I know it's a word but I didn't want to reword everything this is a cancellation rendition
        confirm_refundtion(window.terminal_data.orderId, window.terminal_data.message, backData);
    }
    ...
    function confirm_refundtion(orderId, message, payment_data = '') {
        const data = {
            order_id: orderId,
        ...
        $.ajax({
            ...
            success: function(resp) {
                ...
                if (resp.status == 0) {
                    ...
                    const orderRefundConfirmModal = $('#refund-confirmation');
                    orderRefundConfirmModal.each((index, mod) => {
                        orderRefundConfirmModal.modal('hide');
...

    {
        ...
        window.terminal_data = {orderId: '', message: '', amount: 0};
        $(document).on('click', '.confirm-refund-order', function() {
            const orderId = orderRefundConfirmModal.attr('order');
            const amount = orderRefundConfirmModal.attr('refund_amount');
            if (orderId == 'none') {
                Swal.fire({
            ...
            const agree = orderRefundConfirmModal.find('[name="agreement"]').is(':checked');
            if (!agree) {
                Swal.fire({
                    text: `{{ __('You need to agree with the refund to continue') }}`,
                    icon: "error",
                    buttonsStyling: false,
                    confirmButtonText: `{{ __("Ok, got it!") }}`,
                    customClass: {
            ...
            const message = orderRefundConfirmModal.find('[name="description"]').val();
            if (message.trim() == '') {
                Swal.fire({
                    text: `{{ __('Reason message must be filled to refund an order') }}`,
                    icon: "error",
                    buttonsStyling: false,
                    confirmButtonText: `{{ __("Ok, got it!") }}`,
                    customClass: {
            ...
            if (!amount || parseFloat(amount) == 0) {
                confirm_refundtion(orderId, message);
            } else {
                if (!window.terminal_confirmation) {
                    Swal.fire({
                        text: `{{ __('This order is paid by card the terminal is disabled or not configured, please configure the terminal and refresh the app from POS') }}`,
                        icon: "error",
                        buttonsStyling: false,
                        confirmButtonText: `{{ __("Ok, got it!") }}`,
                        customClass: {
                ...
                if ('Mine' in window) {
                    ...
                    window.Mine.postMessage('ria:' + amount + ':' + getTerminalDetails());
                } else console.log('ria:' + amount + ':' + getTerminalDetails());
            }
        })


        $(document).on('click', '.discard-refund-order', function() {
            orderRefundConfirmModal.attr('order', 'none');
            orderRefundConfirmModal.modal('hide');
        })
    }
    window.terminal_ip = '';
