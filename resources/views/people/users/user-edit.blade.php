                ...
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex justify-content-center flex-wrap me-3">
                        <a href="{{ route('user.index') }}"
                            class="page-heading d-flex text-gray-900 fs-3 flex-column justify-content-center my-0">{{ __('Users') }}
                            > </a>
                        <span
                            class="page-heading d-flex text-info fw-bold fs-3 flex-column justify-content-center my-0 m-4">{{ $person->name }}</span>
                    </div>
            ...
            <div id="kt_app_content" class="app-content flex-column-fluid">
                <div id="kt_app_content_container" class="app-container container-xxl">
                    <form id="kt_ecommerce_add_user_form" class="form d-flex flex-column flex-lg-row">
                        ...
                        <input type="hidden" id="page-id" value="{{ $person->id }}">
                                            ...
                                            <div class="card-body pt-0 pb-0">
                                                <div class="row">
                                                    <div class="mb-5 fv-row col">
                                                        ...
                                                        <input id="name" type="text" name="name"
                                                            value="{{ $person->name }}" class="form-control mb-2"
                                                    ...
                                                    <div class="mb-5 fv-row col">
                                                        ...
                                                        <input id="email" type="email" name="email"
                                                            value="{{ $person->email }}" class="form-control mb-2"
                                                ...
                                                <div class="row">
                                                    <div class="mb-5 fv-row col-6">
                                                        <label for="password"
                                                            class="form-label fw-bold">{{ __('New Password(Optional)') }}</label>
                                                        <div class="shared-input-container mb-2">
                                                        <input id="password" type="password" name="password"
                                                            class="form-control"
                                                            placeholder="{{__("Change the password if you want")}}" />
                                                    ...
                                                    <div class="mb-5 fv-row col-6">
                                                        ...
                                                        <input id="pin" type="password" name="pin"
                                                            class="form-control" value="{{ $person->pin }}"/>
                                                        <span class="toggle-password">
                                                                <i class="fa fa-eye-slash"></i>
                                                            </span>
                                                        </div>
                                                ...
                                                <div class="fv-row col-6">
                                                    ...
                                                    <select @disabled(!$person->location_id) id="location_id" name="location_id" class="form-select"
                                                            data-control="select2"
                                                            data-placeholder="{{ __('Select an location') }}"
                                                            data-allow-clear="false">
                                                        @foreach (\App\Helpers\Helpers::locations() as $location)
                                                            <option {{ $location->id == $person->location_id ? 'selected' : '' }} value="{{ $location->id }}">{{ $location->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="card-header">
                                                <div class="card-title">
                                                    <h2>{{ __('User Settings') }}</h2> <span
                                                        class="small p-5 text-gray-900">{{ __('User access and permission settings.') }}</span>
                                                </div>
                                            </div>
                                            <div class="card-body pt-0 pb-0">
                                                <div class="row">
                                                    <div class="mb-10 fv-row col">
                                                        <label for="roles" class="form-label fw-bold">{{ __('Role') }}</label>
                                                        <select id="roles" name="role_id" class="form-select mb-2"
                                                            ...
                                                            @foreach ($userRoles as $userRole)
                                                                <option
                                                                    {{ $userRole->id == $person->userRole->id ? 'selected' : '' }}
                                                    ...
                                                    <div class="mb-10 fv-row col">
                                                        <label class="form-check-label mb-4"
                                                            for="activated"><strong>{{ __('Status*') }}</strong> <span
                                                                class="small">{{ __('When the user is deactivated, the registry is created in the system, but can not login until it is activated again') }}</span></label>
