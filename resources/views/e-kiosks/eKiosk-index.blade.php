...
@section('setup-script')
    ...
    @vite('resources/assets/js/custom/apps/e-kiosks/list/list.js')
@endsection
@section('page-style')
                    ...
                    <div class="shadow-none bg-transparent border-0">
                        <div class="border-0 px-0 py-8 d-flex justify-content-between">
                            ...
                            <div class="card-toolbar d-flex h-45px">
                                @include('settings.search', ['label' =>'Search eKiosks by name or eKiosk ID'])
                                <div class="d-flex justify-content-end w-125px" data-kt-customer-table-toolbar="base">
                                    <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true"
                                         id="kt-toolbar-filter">
                                        <div class="separator border-gray-200"></div>
                                    </div>
                                    <a href="{{ route('eKiosk.create') }}" class="btn btn-primary w-100 border-0">{{ __('Add new') }}</a>
                                </div>
                        ...
                        <div class="card card-body pt-0">
                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                                ...
                                <tr class="text-start text-gray-600 fs-6 gs-0">
                                    ...
                                    <th class="text-gray-900 fw-bold min-w-125px py-10">{{ __('eKiosk name') }}</th>
                                    <th class="text-gray-900 fw-bold min-w-125px py-10">{{ __('Created by') }}</th>
                                    <th class="text-gray-900 fw-bold min-w-125px py-10">{{ __('eKiosk ID') }}</th>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input" type="checkbox" value="{{ $eKiosk->id }}"/>
                                            </div>
                                        </td>

                                        <td>
                                            <a href="{{ route('eKiosk.edit', ['eKiosk' => $eKiosk->id ]) }}"
                                               class="text-gray-800 text-hover-primary mb-1 fs-10.5 flex-column justify-content-center my-0">{{ $eKiosk->name }}</a>
                                        </td>
                                        <td>
                                            <div
                                                class="text-gray-800 fs-10.5 flex-column justify-content-center my-0">
                                                <a href="{{ route('user.edit', $eKiosk->user?->id ?? '') }}"
                                                    class="text-gray-800 text-hover-primary mb-1">{{ $eKiosk->user?->name }}</a>
                                            </div>
                                        </td>
                                        <td>
                                                <div class="text-gray-800 mb-1 fs-10.5 flex-column justify-content-center my-0">{{ $eKiosk->e_kiosk_id }}</div>
                                        </td>
                                        <td  class="text-gray-800 fs-10.5 flex-column justify-content-center my-0"
                                        style="-webkit-text-security: disc;">{{ substr($eKiosk->authentication_code, 0, 20) }}</td>
                                        <td>
                                            <div
                                            class="text-gray-800 fs-10.5 flex-column justify-content-center my-0">
                                                {{ \Carbon\Carbon::parse($eKiosk->created_at)->format('d.m.Y') }}
                                            </div>
                                        </td>
                                        <td>
                                            <div
                                                    class="text-gray-800 fs-10.5 flex-column justify-content-center my-0">
                                            {{$eKiosk->location?->name }}
                                            </div>
                                        </td>
                                        <td><div class="badge {{ $eKiosk->status ? 'badge-light-success' : 'badge-light-danger'}}">{{ $eKiosk->status ? __('Active') : __('Inactive') }}</div></td>
                                        <td class="text-end">
                                            <a href="#"
