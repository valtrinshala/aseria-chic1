                    ...
                    <div class="page-title d-flex justify-content-center flex-wrap me-3">
                        <a href="{{ route('eKioskAsset.index') }}"
                           class="page-heading d-flex text-gray-900 fs-3 flex-column justify-content-center my-0">{{ __('Manage assets') }}
                            ></a>
                        <span
                            class="page-heading d-flex text-info fw-bold fs-3 flex-column justify-content-center my-0 m-4">{{ $eKioskAsset->name }}</span>
            ...
            <div id="kt_app_content" class="app-content flex-column-fluid">
                <div id="kt_app_content_container" class="app-container container-xxl">
                    <form id="kt_ecommerce_add_eKioskAsset_form" class="form d-flex flex-column flex-lg-row">
                        ...
                        <input type="hidden" id="page-id" value="{{ $eKioskAsset->id }}">
                        <div class="d-flex flex-column gap-7 gap-lg-7 w-100 w-lg-425px mb-7 me-lg-7">
                            <div class="card card-flush py-1">
                                <div class="card-header">
                                    <div class="card-toolbar w-100">
                                        <h2>{{ __('Asset') }}</h2><span class="small p-5 text-muted">{{ __('Only *.png, *.svg, *.webp, *.jpg and *.jpeg image files are accepted') }}</span>
                                    </div>
                                </div>
                                    ...
                                    <div class="card-body py-0 px-0">
                                        <div class="image-input w-100 mb-3" data-kt-image-input="true">
                                            ...
                                            <div class="d-flex justify-content-center">
                                                @if($eKioskAsset->type == 'video')
                                                    <video width="350"  autoplay muted loop>
                                                        <source src="{{ $eKioskAsset->getImage() }}" type="video/mp4">
                                                    </video>
                                                @elseif($eKioskAsset->type == 'image')
                                                    <div class="image-input-wrapper" style="background-image: url('{{ $eKioskAsset->getImage() }}')"></div>
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="card card-flush py-1">
                                            ...
                                            <div class="card-body pt-0">
                                                <div class="row fv-row">
                                                    <div class=" col-6">
                                                        <label class="mt-3 required form-label fw-bold">{{ __('Positions') }}</label>
                                                        <select name="position_id" id="positions" name="e_kiosk_id" class="form-select"
                                                                data-control="select2" data-placeholder="{{ __('Select an option') }}"
                                                                data-allow-clear="true">
                                                            <option></option>
                                                            @foreach ($positions as $position)
                                                                <option {{ $position->id == $eKioskAsset->position_id ? 'selected' : '' }}  value="{{ $position->id }}" data-url="{{ $position->url }}" data-description="{{ $position->description }}" data-status="{{ $position->status }}">{{ $position->name }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    <div class="fv-row col-6 mt-3">
                                                        ...
                                                        <select id="e_kiosk_id" name="e_kiosk_id" class="form-select"
                                                            data-control="select2"
                                                            data-placeholder="{{ __('Select an option') }}"
                                                            data-allow-clear="true">
                                                            <option></option>
                                                            @foreach ($eKiosks as $eKiosk)
                                                                <option {{ $eKiosk->id == $eKioskAsset->e_kiosk_id ? 'selected' : '' }} value="{{ $eKiosk->id }}">{{ $eKiosk->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                ...
                                                <div class="mb-10">
                                                    <div class="row">
                                                        ...
                                                        <h4 class="mt-2 fs-6 fw-normal extend-description">
                                                            {{ $eKioskAsset->position?->description }}
                                                        </h4>
                                                    </div>
                                            ...
                                            <div class="ps-10">
                                                ...
                                                <div class="form-check form-switch">
                                                    ...
                                                    <input class="form-check-input" id="status" name="status"
                                                           type="checkbox" role="checkbox" {{ $eKioskAsset->status ? 'checked' : '' }} value="1">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
