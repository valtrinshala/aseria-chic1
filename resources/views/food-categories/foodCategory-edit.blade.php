                ...
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex justify-content-center flex-wrap me-3">
                        <a href="{{ route('foodCategory.index') }}"
                           class="page-heading d-flex text-gray-900 fs-3 flex-column justify-content-center my-0">{{ __('Update category') }}
                            > </a>
                        <span
                            class="page-heading d-flex text-info fw-bold fs-3 flex-column justify-content-center my-0 m-4">{{ $foodCategory->name }}</span>
                    </div>
                    <div class="d-flex align-items-center gap-2 gap-lg-3">
                        @include('button-language-add-items', [
                            'url' => 'foodCategory.edit',
                            'keyUrl' => 'foodCategory',
                            'valueUrl' => $foodCategory->id,
                        ])
                        <a href="{{ route('foodCategory.index') }}"
            ...
            <div id="kt_app_content" class="app-content flex-column-fluid">
                <div id="kt_app_content_container" class="app-container container-xxl">
                    <form id="kt_ecommerce_add_category_form" class="form d-flex flex-column flex-lg-row">
                        ...
                        <input type="hidden" id="page-id" value="{{ $foodCategory->id }}">
                        <input type="hidden" value="{{ $foodCategory->color }}" name="color" class="input-color">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-9">
                                                <span id="infoSpanColor"
                                                      class="small text-gray-900">{{ __('This modifier and its products will associate with this color') }}</span>
                                            </div>
                                            <div class="col-3">
                                                            ...
                                                            <div class="row colorCategories">
                                                                @foreach($color as $eachColor)
                                                                    <div data-color="{{ $eachColor }}"
                                                                         ...
                                                                         class="col-sm m-1 h-35px cursor-pointer rounded"></div>
                                                                @endforeach
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                ...
                                                <div class="image-input w-100 mb-3" data-kt-image-input="true">
                                                        ...
                                                        <div
                                                            ...
                                                            <div class="d-flex flex-stack flex-grow-1">
                                                                <div class="fw-semibold py-2">
                                                                    <div
                                                                        class="fw-bold">{{__('Quick file uploader')}}</div>
                                                                    <div class=" fs-6 text-gray-700">{{__('Drag & Drop or choose files
                                                                        from computer')}}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                    ...
                                                    <div class="d-flex justify-content-center">
                                                        <div class="image-input-wrapper"
                                                             style="background-image: url('{{ $foodCategory->getImageSystem() }}')"></div>
                                                    </div>
                                                </div>
                                                <div
                                                    class="text-muted fs-7">{{ __('Recommended size')}} {{__(':1080x1080')}}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="tab-pane fade show active" id="kt_ecommerce_add_product_general"
                                    ...
                                    <div class="d-flex flex-column gap-7 gap-lg-7">
                                        <div class="card card-flush py-4">
                                            ...
                                            <div class="card-body pt-0 pb-1">
                                                <div class="mb-10 fv-row">
                                                    <div class="row">
                                                        <label
                                                            class="col required form-label fw-bold">{{ __('Category Title') }}</label>
                                                        <span class="col text-end text-gray-900 fs-7">
                                                            {{ __('A category name is required and recommended to be unique.') }}
                                                        </span>
                                                        <input type="text" name="name" class="form-control mb-2 ms-2"
                                                               placeholder="{{__('Category name')}}"
                                                               value="{{ !request()->query('language_update_id') ? $foodCategory->getAttributes()['name']: $foodCategory->translate?->name }}"/>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="card-body pt-0 pb-0">
                                                <div class="mb-10">
                                                    ...
                                                    <textarea name="description" class="form-control mb-3" rows="4"
                                                              placeholder="{{ __('Type a message') }}">{{ !request()->query('language_update_id') ? $foodCategory->getAttributes()['description']: $foodCategory->translate?->description }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="card card-flush py-3">
                                            ...
                                            <div class="card-header">
                                                <div class="card-title">
                                                    <h2>{{ __('Category for kitchen and eKiosk') }}</h2>
                                                </div>
                                            </div>
                                            <div class="card-body pt-0 pb-0">
                                                <div class="row">
                                                    <div class="mb-10 col">
                                                        ...
                                                        <div class="form-check form-switch">
                                                            ...
                                                            <input class="form-check-input" id="category_for_kitchen" name="category_for_kitchen"
                                                                   type="checkbox" role="checkbox" @checked($foodCategory->category_for_kitchen) value="1">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                    ...
                                                    <div class="mb-10 col">
                                                        ...
                                                        <div class="form-check form-switch">
                                                            ...
                                                            <input class="form-check-input" id="category_to_ask_for_extra_kitchen" name="category_to_ask_for_extra_kitchen"
                                                                   type="checkbox" role="checkbox" @checked($foodCategory->category_to_ask_for_extra_kitchen) value="1">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                    ...
                                                    <div class="mb-10 col">
                                                        ...
                                                        <div class="form-check form-switch mt-4">
                                                            ...
                                                            <input class="form-check-input" id="category_for_pos" name="category_for_pos"
                                                                   type="checkbox" role="checkbox" @checked($foodCategory->category_for_pos) value="1">
                                                        </div>
                                                    </div>
                                                    <div class="mb-10 col">
                                                        ...
                                                        <div class="form-check form-switch mt-4">
                                                            ...
                                                            <input class="form-check-input" id="category_for_kiosk" name="category_for_kiosk"
                                                                   type="checkbox" role="checkbox" @checked($foodCategory->category_for_kiosk) value="1">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
