...
@section('title', __('Kitchens'))
@section('setup-script')
    <script src="{{ asset('assets/plugins/custom/datatables/datatables.bundle.js') }}"></script>
    <script src="{{ asset('assets/js/widgets.bundle.js') }}"></script>
    @vite('resources/assets/js/custom/apps/kitchen/admin/list/list.js')
@endsection
@section('page-style')
                    ...
                    <div class="shadow-none bg-transparent border-0">
                        <div class="border-0 px-0 py-8 d-flex justify-content-between">
                            ...
                            <div class="card-toolbar d-flex h-45px">
                                @include('settings.search', ['label' =>'Search kitchens by name or kitchen ID'])
                                <div class="d-flex justify-content-end w-125px" data-kt-customer-table-toolbar="base">
                                    <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true"
                                         id="kt-toolbar-filter">
                                        <div class="separator border-gray-200"></div>
                                    </div>
                                    <a href="{{ route('devices.create') }}" class="btn btn-primary w-100 border-0">{{ __('Add new') }}</a>
                                </div>
                        ...
                        <div class="card card-body pt-0">
                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                                ...
                                <tr class="text-start text-gray-600 fs-6 gs-0">
                                    ...
                                    <th class="text-gray-900 fw-bold min-w-125px py-10">{{ __('Kitchen name') }}</th>
                                    <th class="text-gray-900 fw-bold min-w-125px py-10">{{ __('Created by') }}</th>
                                    <th class="text-gray-900 fw-bold min-w-125px py-10">{{ __('Kitchen ID') }}</th>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                <input class="form-check-input" type="checkbox" value="{{ $kitchen->id }}"/>
                                            </div>
                                        </td>

                                        <td>
                                            <a href="{{ route('devices.edit', ['device' => $kitchen->id ]) }}"
                                               class="text-gray-800 text-hover-primary mb-1 fs-10.5 flex-column justify-content-center my-0">{{ $kitchen->name }}</a>
                                        </td>
                                        <td>
                                            <div
                                                class="text-gray-800 fs-10.5 flex-column justify-content-center my-0">
                                                <a href="{{ route('user.edit', $kitchen->user?->id ?? '') }}"
                                                    class="text-gray-800 text-hover-primary mb-1">{{ $kitchen->user?->name }}</a>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="text-gray-800 mb-1 fs-10.5 flex-column justify-content-center my-0">{{ $kitchen->kitchen_id }}</div>
                                        </td>
                                        <td  class="text-gray-800 fs-10.5 flex-column justify-content-center my-0"
                                        style="-webkit-text-security: disc;">{{ substr($kitchen->authentication_code, 0, 20) }}</td>
                                        <td>
                                            <div
                                            class="text-gray-800 fs-10.5 flex-column justify-content-center my-0">
                                                {{ \Carbon\Carbon::parse($kitchen->created_at)->format('d.m.Y') }}
                                            </div>
                                        </td>
                                        <td>
                                            <div
                                        ...
                                        <td><div class="badge {{ $kitchen->status ? 'badge-light-success' : 'badge-light-danger'}}">{{ $kitchen->status ? __('Active') : __('Inactive') }}</div></td>
                                        <td class="text-end">
                                            <a href="#"
