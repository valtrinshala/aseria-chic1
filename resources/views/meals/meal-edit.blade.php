...
@section('title', 'Update Meal')
@section('setup-script')
    @vite('resources/assets/js/custom/apps/meals/update.js')
@endsection
@section('content')
    <div class="app-main flex-column flex-row-fluid" id="kt_app_main">
        <div class="d-flex flex-column flex-column-fluid m-9 mt-0">
                ...
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex justify-content-center flex-wrap me-3">
                        <a href="{{ route('meal.index') }}"
                           class="page-heading d-flex text-gray-900 fs-3 flex-column justify-content-center my-0">{{ __('Update Deal') }}
                            > </a>
                        <span
                            class="page-heading d-flex text-info fw-bold fs-3 flex-column justify-content-center my-0 m-4">{{ $meal->name }}</span>
            ...
            <input type="hidden" id="pageId" value="{{ $meal->id }}">
                ...
                <div id="kt_app_content_container" class="app-container container-xxl">
                    <form id="kt_ecommerce_add_meal_form" class="form d-flex flex-column flex-lg-row">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-9 d-none">
                                                <span id="infoSpan" class="small text-gray-900"
                                                      style="display:{{ $meal->category->color }};">{{ __('This modifier and its products will associate with this image') }}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="tab-content">
                                            <div class="tab-pane fade active show"
                                                ...
                                                <div class="row ms-0 mb-7">
                                                    ...
                                                    <div class="col-10">
                                                        <span
                                                            class="small">{{ __('The deals of this category will associate with this color') }}</span>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="image-input w-100 mb-3" data-kt-image-input="true">
                                                    ...
                                                    <div class="d-flex justify-content-center">
                                                        <div class="image-input-wrapper"
                                                             style="background-image: url('{{ $meal->getImage() }}')">
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <div class="card-body pt-0 pb-1">
                                    <div class="fv-row">
                                        ...
                                        <select id="categories" name="categories" class="form-select mb-2"
                                            ...
                                            <option
                                                {{ $category->id == $meal->category?->id ? 'selected' : '' }} data-color="{{ $category->color }}"
                                    ...
                                    <div class="fv-row">
                                        ...
                                        <select id="tax_id" name="tax_id" class="form-select mb-2"
                                            ...
                                            @foreach ($taxes as $tax)
                                                <option @selected($tax->id == $meal->tax_id) value="{{ $tax->id }}">
                                                    {{ $tax->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            <div class="card card-flush py-4">
                                ...
                                <div class="card-body pt-0 pb-1">
                                    ...
                                    <select name="status" class="form-select mb-2" data-control="select2"
                                        ...
                                        <option></option>
                                        <option {{ $meal->status ? 'selected' : '' }} value="1">
                                            {{ __('Published') }}</option>
                                        <option {{ !$meal->status ? 'selected' : '' }} value="0">
                                            {{ __('Inactive') }}</option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-7">
                            <div class="tab-content">
                                <div class="tab-pane fade show active" id="kt_ecommerce_add_meal_general"
                                     role="tab-panel">
                                                ...
                                                <div class="mb-10 fv-row">
                                                    <div class="row">
                                                        ...
                                                        <span class="col text-end text-gray-900 fs-7">
                                                            {{ __('A deal name is required and recommended to be unique.') }}
                                                        </span>
                                                        <input type="text" name="name"
                                                               class="form-control mb-2 ms-2"
                                                               placeholder="{{ __('Deal name') }}"
                                                               value="{{ !request()->query('language_update_id') ? $meal->getAttributes()['name']: $meal->translate?->name }}"/>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="card-body pt-0 pb-0">
                                                <div class="mb-10">
                                                    ...
                                                    <textarea name="description" class="form-control mb-3" rows="4"
                                                              placeholder="{{ __('Type a message') }}">{{ !request()->query('language_update_id') ? $meal->getAttributes()['description']: $meal->translate?->description }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="card-header">
                                                ...
                                                <div
                                                    ...
                                                    <select class="form-select mb-2" id="foodItemDropdown"
                                                            ...
                                                            data-placeholder="{{ __('Search food items by name or food item ID') }}"
                                                            data-allow-clear="true">
                                                        <option></option>
                                                    ...
                                                    <div class="card card-body pt-0 px-2 pb-0">
                                                        <table class="table align-middle table-row-dashed gy-5"
                                                            ...
                                                            @foreach ($meal->foodItems as $foodItem)
                                                                <tr id="{{ $foodItem->id }}">
                                                                    <input id="qty-{{ $foodItem->id }}"
                                                                           type="hidden"
                                                                           name="foodItems[{{ $foodItem->id }}]"
                                                                           value="{{ $foodItem->pivot->quantity }}">
                                                                    <td>
                                                                        <a href="{{ route('foodItem.edit', $foodItem->id) }}"
                                                                           class="text-gray-800 text-hover-primary mb-1">{{ $foodItem->name }}</a>
                                                                    </td>
                                                                    <td data-qty="1">
                                                                        {{ $foodItem->pivot->quantity }}</td>
                                                                    <td data-cost="{{ $foodItem->cost }}">
                                                                        @if(!$settings->currency_symbol_on_left)
                                                                            {{ $foodItem->cost * $foodItem->pivot->quantity }}
                                                                    ...
                                                                    <td data-price="{{ $foodItem->price }}">
                                                                        @if(!$settings->currency_symbol_on_left)
                                                                            {{ $foodItem->price * $foodItem->pivot->quantity }}
                                                                            {{ $settings->currency_symbol }}
                                                                        @else
                                                                            {{ $settings->currency_symbol }}
                                                                            {{ $foodItem->price * $foodItem->pivot->quantity }}
                                                                    ...
                                                                    <td class="float-right pe-0">
                                                                        <div class="float-end menu-item px-3 pe-0"><a
