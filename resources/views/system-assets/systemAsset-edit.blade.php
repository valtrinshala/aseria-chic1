...
@section('setup-script')
    @vite('resources/assets/js/custom/apps/system-assets/add.js')
@endsection
@section('content')
        ...
        <div class="d-flex flex-column flex-column-fluid m-9 mt-0">
            <div id="kt_app_toolbar" class="app-toolbar px-0 py-8">
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex justify-content-center flex-wrap me-3">
                        <a href="{{ route('systemAsset.index') }}"
                           class="page-heading d-flex text-gray-900 fs-3 flex-column justify-content-center my-0">{{ __('Manage assets') }}
                            ></a>
                        <span
                            class="page-heading d-flex text-info fw-bold fs-3 flex-column justify-content-center my-0 m-4">{{ $systemAsset->name }}</span>
            ...
            <div id="kt_app_content" class="app-content flex-column-fluid">
                <div id="kt_app_content_container" class="app-container container-xxl">
                    <form id="kt_ecommerce_add_systemAsset_form" class="form d-flex flex-column flex-lg-row">
                        <input type="hidden" id="method" value="update">
                        <input type="hidden" id="page-id" value="{{ $systemAsset->id }}">
                                    ...
                                    <div class="card-body py-0 px-0">
                                        <div class="image-input w-100 mb-3" data-kt-image-input="true">
                                            ...
                                            <div class="d-flex justify-content-center">
                                                <div class="image-input-wrapper" style="background-image: url('{{ $systemAsset->getImage() }}')"></div>
                                            </div>
                                        </div>
                                                ...
                                                <div class="row fv-row">
                                                    <div class="mb-10 fv-row col-6">
                                                        ...
                                                        <input type="text" name="name" class="form-control mb-2"
                                                               placeholder="Category name" value="{{ $systemAsset->name }}"/>
                                                    </div>
                                                    <div class="fv-row col-6 mt-3">
                                                        <label
                                                            class="required form-label fw-bold">{{ __('Cash registers') }}</label>
                                                        <select id="cash_register_id" name="cash_register_id" class="form-select"
                                                                data-control="select2"
                                                                data-placeholder="{{ __('Select an option') }}"
                                                                data-allow-clear="true">
                                                            <option></option>
                                                            @foreach ($cashRegisters as $cashRegister)
                                                                <option @selected($cashRegister->id == $systemAsset->cash_register_id) value="{{ $cashRegister->id }}">{{ $cashRegister->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                            ...
                                            <div class="ps-10">
                                                ...
                                                <div class="form-check form-switch">
                                                    ...
                                                    <input class="form-check-input" id="status" name="status"
                                                           type="checkbox" role="checkbox" {{ $systemAsset->status ? 'checked' : '' }} value="1">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
