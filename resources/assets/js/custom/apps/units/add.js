...
var KTModalCustomersAdd = function () {
    ...
    var handleForm = function () {
            ...
            form,
            {
                fields: {
                    ...
                    'suffix': {
                        validators: {
                            notEmpty: {
                                message: window.keys.unitSuffix
                            }
                        }
                    }
                },
        ...
        $('.delete-button').click(function (event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "/admin/setting/unit/" + recordId,
                        method: "DELETE",
                        contentType: false,
                        processData: false,
                        headers: {
                        ...
                        success: function (result) {
                            location.href = '/admin/setting/unit/'
                        },
        ...
        $('#submitButton').on('click', function () {
            $('#kt_ecommerce_add_unit_form').submit();
        });
        $('#kt_ecommerce_add_unit_form').submit(function (e) {
            var formData = new FormData($('#kt_ecommerce_add_unit_form')[0]);
            if ($('#method').val() === 'create'){
                var url =  '/admin/setting/unit';
            }else if ($('#method').val() === 'update' ){
                url =  '/admin/setting/unit/'+$('#page-id').val();
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                Swal.fire({
                                    ...
                                    if ($('#method').val() === 'create'){
                                        $('#kt_ecommerce_add_unit_form')[0].reset();
                                    }
                                });
                            },
                            statusCode: {
    ...
    return {
        init: function () {
            form = document.querySelector('#kt_ecommerce_add_unit_form');
