...
var KTModalCustomersAdd = function () {
    ...
    var handleForm = function () {
            ...
            form,
            {
                fields: {
                    'name': {
                        validators: {
                            notEmpty: {
                                message: window.keys.productNameRequired
                            }
                        }
                    },
                    'ingredients[]': {
        ...
        $('#activated').change(function () {
            if ($(this).is(':checked')) {
                ...
                table.find('.large').css('display', 'none')
                table.find('.medium').css('display', '').removeClass('size-table-switch')
                table.find('.small').css('display', '').removeClass('size-table-switch')
                table.find('#medium').prop('checked', true)
                table.find('#small').prop('checked', true)
                $('.medium input[type="number"]').prop('disabled', false).val(0);
            } else {
        ...
        $('#large').change(function () {
            if ($(this).is(':checked')) {
                table.find('.large').css('display', '').removeClass('size-table-switch')
            } else {
                table.find('.large').css('display', 'none')
                table.find('.large').val(0)
            }
        });
        $('#small').change(function () {
            if ($(this).is(':checked')) {
                table.find('.small').css('display', '').removeClass('size-table-switch')
                $('.medium input[type="number"]').prop('disabled', false).val(0);
                $('.large input[type="number"]').prop('disabled', false);
            } else {
                table.find('.small').css('display', 'none')
                $('.medium input[type="number"]').prop('disabled', true).val(0);
                table.find('.small').val(0)
                if (!table.find('#medium').is(':checked')){
                    $('.large input[type="number"]').prop('disabled', true).val(0);
                }
            }
        });
        $('#medium').change(function () {
            if ($(this).is(':checked')) {
                table.find('.medium').css('display', '').removeClass('size-table-switch')
                $('.large input[type="number"]').prop('disabled', false);
                if (!table.find('#small').is(':checked')){
                    $('.medium input[type="number"]').prop('disabled', true).val(0);
                }
            } else {
                table.find('.medium').css('display', 'none')
                table.find('.medium').val(0)
                if (!table.find('#small').is(':checked')){
                    $('.large input[type="number"]').prop('disabled', true).val(0);
                }
            }
        ...
        $('#ingredientDropdown').on('change', function () {
            let selectedIng = JSON.parse($(this).val());
            let urlEdit = "/admin/ingredient/" + selectedIng.id + "/edit"
            if (!selectedIng) {
                return;
            }
        ...
        $(document).ready(function () {
            ...
            let secondIconDiv = $('#secondIconDiv');
            let secondSpanDiv = $('#secondSpanDiv');
            ...
            $('#kt_security_summary_tabs a[data-bs-toggle="tab"]').click(function () {
                ...
                switch (targetTabId) {
                    ...
                    case "#kt_security_summary_tab_pane_day":
                        ...
                        secondSpanDiv.hide();
                        break;
                    case "#kt_security_summary_tab_pane_second":
                        colorDiv.hide();
                        infoSpanColor.hide();
                        iconDiv.hide();
                        infoSpanDiv.hide();
        ...
        $('#kt_ecommerce_add_product_form').submit(function (e) {
            let checkboxIds = ['small', 'medium', 'large'];
            let selectedCount = 0;
                ...
                validator.validate().then(function (status) {
                    if (status == 'Valid') {
                        if (selectedCount === 1){
                            ...
                            let formData = new FormData($('#kt_ecommerce_add_product_form')[0]);
                            formData.append('_method', 'PUT');
                            $.ajax({
                                url: '/admin/foodItem/' + $('#pageId').val(),
                                type: 'POST',
                                data: formData,
                                contentType: false,
                                processData: false,
                                ...
                                success: function (response) {
                                    Swal.fire({
                                        text: window.keys.foodItemUpdated,
                                        icon: "success",
                                        buttonsStyling: false,
                                        confirmButtonText: window.keys.confirmButtonOk,
                                        customClass: {
    ...
    return {
        init: function () {
            form = document.querySelector('#kt_ecommerce_add_product_form');
