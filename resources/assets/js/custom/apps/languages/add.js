...
var KTModalCustomersAdd = function () {
    ...
    var handleForm = function () {
            ...
            form,
            {
                fields: {
                    'name': {
                        validators: {
                            notEmpty: {
                                message: window.keys.languageName
                            }
                        }
                    },
                    'locale': {
                        validators: {
                            notEmpty: {
                                message: window.keys.localeName
                            }
                        }
                    }
                },
        ...
        $('#submitButton').on('click', function () {
            $('#kt_ecommerce_add_language_form').submit();
        });
        $('#kt_ecommerce_add_language_form').submit(function (e) {
            var formData = new FormData($('#kt_ecommerce_add_language_form')[0]);
            if ($('#method').val() === 'create'){
                var url =  '/admin/language';
            }else if ($('#method').val() === 'update' ){
                url =  '/admin/language/'+$('#page-id').val();
                    ...
                    if (status == 'Valid') {
                        $.ajax({
                            ...
                            success: function (response) {
                                Swal.fire({
                                    text: window.keys.languageAdded,
                                    icon: "success",
                                    buttonsStyling: false,
                                    confirmButtonText: window.keys.confirmButtonOk,
                                    customClass: {
                                    ...
                                    if ($('#method').val() === 'create'){
                                        window.location = '/admin/language/'+response.id+'/edit';
                                    } else {
                                        document.body.dir = formData.get('direction');
                                    }
                                });
                            },
                            statusCode: {
    ...
    return {
        init: function () {
            form = document.querySelector('#kt_ecommerce_add_language_form');
