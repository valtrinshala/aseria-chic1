...
var KTModalCustomersAdd = function () {
    ...
    var handleForm = function () {
            ...
            form,
            {
                fields: {
                    'name': {
                        validators: {
                            notEmpty: {
                                message: window.keys.categoryNameRequired
                            }
                        }
                    }
                },
        ...
        $('.colorCategories').children().click(function() {
            $('.input-color').val($(this).data('color'))
            $('.category-color').css('background-color', $(this).data('color'));
        });
        ...
        $('#kt_ecommerce_add_category_form').submit(function (e) {
            var formData = new FormData($('#kt_ecommerce_add_category_form')[0]);
            if ($('#method').val() === 'create'){
                var url =  '/admin/foodCategory';
            }else if ($('#method').val() === 'update' ){
                url =  '/admin/foodCategory/'+$('#page-id').val();
                    ...
                    if (status == 'Valid') {
                        $.ajax({
                            ...
                            success: function (response) {
                                Swal.fire({
                                    text: window.keys.categorySuccessfullyAdded,
                                    icon: "success",
                                    ...
                                    if ($('#method').val() === 'create'){
                                        ...
                                        location.href = '/admin/foodCategory'
                                    }
                                });
                            },
                            statusCode: {
