...
var KTCustomersList = function () {
    ...
    var handleDeleteRows = () => {
        ...
        deleteButtons.forEach(d => {
            d.addEventListener('click', function (e) {
                e.preventDefault();
                const foodCategoryId = e.target.getAttribute('data-food-category-id');
                const parent = e.target.closest('tr');

                const foodCategoryName = parent.querySelectorAll('td')[1].innerText;
                console.log(foodCategoryId);

                Swal.fire({
                    text: window.keys.deleteText + foodCategoryName + "?",
                    icon: "warning",
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "/admin/foodCategory/"+foodCategoryId,
                            method: "DELETE",
                            contentType: false,
                            processData: false,
                            headers: {
                        ...
                        Swal.fire({
                            text: foodCategoryName + window.keys.notDeleted,
                            icon: "error",
                            buttonsStyling: false,
                            confirmButtonText: window.keys.confirmButtonOk,
                            customClass: {
    ...
    var initToggleToolbar = () => {
        ...
        deleteSelected.addEventListener('click', function () {
            Swal.fire({
                text: window.keys.deleteSelectedCategories,
                icon: "warning",
                showCancelButton: true,
                buttonsStyling: false,
                confirmButtonText: window.keys.confirmButtonText,
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/admin/foodCategory/delete/selected",
                        method: "POST",
                        data: { ids: ids },
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    ...
                    Swal.fire({
                        text: window.keys.selectedCategories,
                        icon: "error",
                        buttonsStyling: false,
                        confirmButtonText: window.keys.confirmButtonOk,
                        customClass: {
