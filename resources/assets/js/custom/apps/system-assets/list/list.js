    ...
    var handleDeleteRows = () => {
        ...
        deleteButtons.forEach(d => {
            d.addEventListener('click', function (e) {
                e.preventDefault();
                const systemAssetId = e.target.getAttribute('data-systemAsset-id');
                const parent = e.target.closest('tr');

                const systemAssetName = parent.querySelectorAll('td')[1].innerText;

                Swal.fire({
                    text: window.keys.deleteText + ' ' + systemAssetName + "?",
                    icon: "warning",
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "/admin/systemAsset/"+systemAssetId,
                            method: "DELETE",
                            contentType: false,
                            processData: false,
                            headers: {
                        ...
                        Swal.fire({
                            text: systemAssetName + " was not deleted.",
                            icon: "error",
                            buttonsStyling: false,
        ...
        deleteSelected.addEventListener('click', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/admin/systemAsset/delete/selected",
                        method: "POST",
                        data: { ids: ids },
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    ...
                    Swal.fire({
                        text: window.keys.selectedAsset,
                        icon: "error",
                        buttonsStyling: false,
                        confirmButtonText: window.keys.confirmButtonOk,
                        customClass: {
