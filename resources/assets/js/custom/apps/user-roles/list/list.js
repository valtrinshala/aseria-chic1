    ...
    var handleDeleteRows = () => {
        ...
        deleteButtons.forEach(d => {
            d.addEventListener('click', function (e) {
                e.preventDefault();
                const userRoleId = e.target.getAttribute('data-userRole-id');
                const parent = e.target.closest('tr');

                const userRoleName = parent.querySelectorAll('td')[1].innerText;

                Swal.fire({
                    text: window.keys.deleteText + ' ' + userRoleName + "?",
                    icon: "warning",
                    ...
                    if (result.value) {
                        ...
                        Swal.fire({
                            text: userRoleName + ' ' + window.keys.notDeleted,
                            icon: "error",
                            buttonsStyling: false,
                            confirmButtonText: window.keys.confirmButtonOk,
                            customClass: {
        ...
        deleteSelected.addEventListener('click', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/admin/userRole/delete/selected",
                        method: "POST",
                        data: { ids: ids },
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    ...
                    Swal.fire({
                        text: "Selected user roles was not deleted.",
                        icon: "error",
                        buttonsStyling: false,
                        confirmButtonText: window.keys.confirmButtonOk,
                        customClass: {
