...
var KTModalCustomersAdd = function () {
    ...
    var handleForm = function () {
        ...
        $('#activated').change(function () {
            if ($(this).is(':checked')) {
                $('.activate-other-settings').removeClass('d-none');
            } else {
                $('.activate-other-settings').addClass('d-none');
            }
        });
        ...
        $('.delete-button').click(function (event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "/admin/setting/tax/" + recordId,
                        method: "DELETE",
                        contentType: false,
                        processData: false,
                        headers: {
                        ...
                        success: function (result) {
                            location.href = '/admin/setting/tax/'
                        },
        ...
        $('#submitButton').on('click', function () {
            $('#kt_ecommerce_add_tax_form').submit();
        });
        $('#kt_ecommerce_add_tax_form').submit(function (e) {
            var formData = new FormData($('#kt_ecommerce_add_tax_form')[0]);
            if ($('#method').val() === 'create'){
                var url =  '/admin/setting/tax';
            }else if ($('#method').val() === 'update' ){
                url =  '/admin/setting/tax/'+$('#page-id').val();
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                Swal.fire({
                                    ...
                                    if ($('#method').val() === 'create'){
                                        $('#kt_ecommerce_add_tax_form')[0].reset();
                                    }else {
                                        location.href = '/admin/setting/tax'
                                    }
                                });
    ...
    return {
        init: function () {
            form = document.querySelector('#kt_ecommerce_add_tax_form');
